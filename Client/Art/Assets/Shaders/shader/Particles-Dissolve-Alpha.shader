...
Shader "Shader/Particles-Dissolve-Alpha" {
    Properties {
    ...
    SubShader 
	{
		...
		Pass 
		{
            ...
            float4 frag(VertexOutput i) : COLOR 
			{
				...
				//float2 node_9681 = (i.uv0 + node_2757.g*float2(0,0.2));
				//float4 _1_var = tex2D(_1,TRANSFORM_TEX(node_9681, _1));
				//float2 node_2002 = (i.uv0 + node_2757.g*float2(0,-0.1));
				//float4 _2_var = tex2D(_2,TRANSFORM_TEX(node_2002, _2));
				//float2 node_8467 = (i.uv0 + node_2757.g*float2(-0.15,-0.05));
				//float4 _3_var = tex2D(_3,TRANSFORM_TEX(node_8467, _3));
				//float2 node_4524 = (i.uv0 + node_2757.g*float2(0.2,0.05));
				//float4 _4_var = tex2D(_4,TRANSFORM_TEX(node_4524, _4));
				...
				//float2 sceneUVs = float2(1,grabSign)*i.screenPos.xy*0.5 + 0.5 + ((((_Normal_var.rgb*node_2204)*node_2204).rg*i.vertexColor.a)*(_Nor_Refraction*_Refraction_Factor));
