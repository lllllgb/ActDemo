...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        public GameObject Prefba {
            get {
                if (prefabSource.AddComponent())
                {
                    return prefabSource.AddComponent();
                }
                return AddPrefba();
                ; } }
        GameObject AddPrefba()
        {
           prefabSource.prefab = this.transform.Find(prefabSource.prefabName).gameObject;
            return prefabSource.prefab;
        }
        ...
        public void Dispose()
        {
            for (int i = 0; i < content.childCount; i++)
            {
                dataSource.Dispose(content.GetChild(i));
            }
        }
